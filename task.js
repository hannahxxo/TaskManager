...
What would like to do (Please enter one of the options below):
...
// Displays the menu for the user to select an option. Also, sets the user's response to the userInput variable. Also, converting value to upper case
let userInput = prompt(startMenu).toUpperCase();

// Loops and continues to display menu until the user ends the program (AKA until the the user enters CLOSE. (Uppercase or lowercase))
while (userInput !== "CLOSE") {
    ...
    if (userInput === "TASKS") {
        ...
        alert(showTasks);
        // Sets the value of the showTasks variable back to an empty string
        showTasks = "";
        ...
        while (true) {
            // Concatenates each task/item in the tasks array to the showTasks string variable. Also, sets a number/index for each task.
            for (i = 0; i < tasks.length; i++) {
                // Using \n to create a new line. Also, adding 1 to i (AKA The index) so the numbers will start displaying at 1.
                showTasks += `${i + 1}: ${tasks[i]}\n`;
            }
            // Prompts the user to enter a number and stores their response. Also, using /n to create a new line. Finally, subtracting 1 from the user's entry so it matches the index in the tasks array. (NOTE: JavaScript will attempt to convert a string into a number when you try to subract from it. Just like ParseInt is not actually necessary here.)
            ...
            // Alerts user of with the task/item that has been removed (NOTE: Using index on the "removed" variable because splice returns what is removed in an array)
    ...
    userInput = prompt(startMenu).toUpperCase();
}
